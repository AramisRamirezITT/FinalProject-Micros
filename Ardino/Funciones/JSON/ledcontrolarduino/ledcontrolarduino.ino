...
void loop() {
  ...
  if (client.connect("192.168.0.15",8080)) { // Preguntamos se si conectó a la IP del servidor 
    ...
    Serial.println( String(doc["usuario"]));
    Serial.println(doc["led"].as<int>());
    Serial.println(doc["led1"].as<int>());
    Serial.println(doc["led2"].as<int>());

    if(doc["led"].as<int>() == 255){digitalWrite(LED, HIGH);}else{ digitalWrite(LED, LOW); }
    if(doc["led1"].as<int>() == 255){digitalWrite(LED1, HIGH);}else{ digitalWrite(LED1, LOW); }
    if(doc["led2"].as<int>() == 255){digitalWrite(LED2, HIGH);}else{ digitalWrite(LED2, LOW); }
